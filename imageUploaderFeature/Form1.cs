            ...
            using (cn = new SqlConnection(@"Data Source=LAB109PC23\SQLEXPRESS; Initial Catalog=userRecords; Integrated Security=True;"))
            {
                ...
                if (!string.IsNullOrEmpty(password.Text) && !string.IsNullOrEmpty(rePassword.Text))
                {
                    if (password.Text == rePassword.Text)
                    {
                        ...
                        using (cmd = new SqlCommand("SELECT * FROM userInfo WHERE username=@username", cn))
                        {
                            ...
                            using (dr = cmd.ExecuteReader())
                            {
                                ...
                                else
                                {
                                    ...
                                    using (cmd = new SqlCommand("INSERT INTO userInfo (username, email, password, img) VALUES (@username, @email, @password, NULLIF(CONVERT(varbinary(max), @img), ''))", cn))
                                    {
                                        cmd.Parameters.AddWithValue("@username", username.Text);
